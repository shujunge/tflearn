...
network = fully_connected(network, 10, activation='softmax',restore=False)
network = regression(network, optimizer='adam',
                     loss='categorical_crossentropy',
                     learning_rate=0.001)

# Training
model = tflearn.DNN(network, checkpoint_path='model_finetuning',
                    max_checkpoints=3, tensorboard_verbose=0)
model.load('./cifar10_cnn')

model.fit(X, Y, n_epoch=10, validation_set=(X_test, Y_test), shuffle=True,
